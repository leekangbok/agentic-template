...
ENV_INFO = ROOT / "docs" / "generated" / "ENV_INFO.md"
...
def build_auto_block() -> str:
    ...
    backend_files = count_entries(BACKEND)
    frontend_files = count_entries(FRONTEND)
...
- `docs/generated/ENV_INFO.md` 존재 여부: {"yes" if ENV_INFO.exists() else "no"}
- `backend/` 파일 수: {backend_files}
- `frontend/` 파일 수: {frontend_files}
...
- 프로젝트 유형: 풀스택 AI 에이전트 협업 템플릿
- 현재 단계: 초기 템플릿 구성 완료 (Go + React + MySQL)
- 목표: AI 에이전트 기반 새 프로젝트를 시작할 때 재사용 가능한 기준 구조 제공
- 상태: 문서와 헬퍼 스크립트, 스타터 코드는 준비되었지만 실제 비즈니스 로직은 아직 없음

## 현재 존재하는 것

- 핵심 문서: `README.md`, `AI_RULES.md`, `CONTEXT.md`, `TASKS.md`
- 환경 정보 스냅샷: `docs/generated/ENV_INFO.md`
- 세션 시작 프롬프트: `startup`
- 아키텍처 문서: `docs/architecture.md`
- 컨텍스트 갱신 스크립트: `scripts/auto_context.py`
- 실행 헬퍼 스크립트: `scripts/run.sh`, `scripts/run.bat`
- 백엔드 스타터 (Go/Gin): `backend/`
- 프런트엔드 스타터 (React/Vite): `frontend/`
...
- `backend/`, `frontend/`는 아직 스타터 상태임
- 로컬 MySQL 연결 설정 확인 필요
...
4. 첫 비즈니스 로직 API와 UI를 수정한 뒤 하네스로 검증합니다.
...
def main() -> None:
    # 환경 정보 갱신
    env_script = ROOT / "scripts" / "env_info.py"
    if env_script.exists():
        print("환경 정보 갱신 중...")
        subprocess.run([sys.executable, str(env_script)], check=False)
